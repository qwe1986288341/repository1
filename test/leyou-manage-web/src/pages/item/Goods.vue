    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="saleable(props.item)">下架</v-btn>
  ...
  export default {
    ...
    methods: {
      ...
      addGoods() {
        ...
        deleteGoods(oldBrand){
            // 根据品牌信息查询商品分类
            this.$http.get("/item/goods/deleteById?id=" + oldBrand.id)
                .then(({data}) => {
                    this.getDataFromServer();
                })
        },
        saleable(oldBrand){
            // 根据品牌信息查询商品分类
            this.$http.get("/item/goods/saleable?id=" + oldBrand.id)
                .then(({data}) => {
                    this.getDataFromServer();
                })
        },
